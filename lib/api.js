 ...
 * @returns {json} - json object of games
 */
export async function getNewReleases(numGame = 7,  range = 45, date = new Date(),) {
 ...
 * 
 * @param {int} numGames - number of games to return
 * @param  {int} metaLow - lower bound of metacritic score
 * @param  {int} metaHigh - upper bound of metacritic score
 * @returns  {json} - json object of games
 */
export async function getRecommendedGames(numGames = 50, metaLow = 80, metaHigh = 100) {
    try {
        let params = {
            key: API_KEY,
            page_size: String(numGames),
            metacritic: `${metaLow},${metaHigh}`,
            ordering: '-rating'
        }
 ...
 * 
 * */
export async function searchGames(query = "", numGames = 5) {
    // Check if there is a search query provided
    if (!query.trim()) return []; // If no query, return an empty array
    
    // Prepare search parameters
    const searchParams = {
        search: query, // The search term
        page_size: numGames, // Number of games to return per page
    };
    
    // Call getGamesWithParams with the search parameters
    try {
        const games = await getGamesWithParams(searchParams);
        console.log(games);
        return games;
    } catch (error) {
 ...
 * @returns {json} - json object of game
 * 
 */
export async function getGameDetails(id = '324997') {
    id = String(id);
    console.log(`${API_URL_GAMES}/${id}`)
    let url = new URL(`${API_URL_GAMES}/${id}`);
...
export async function getGameScreenshots(id = '324997', numScreenshots = 5) {
    id = String(id);
    console.log(`${API_URL_GAMES}/${id}/screenshots`)
    let url = new URL(`${API_URL_GAMES}/${id}/screenshots`);
    url.searchParams.append('key', API_KEY);
    url.searchParams.append('page_size', numScreenshots);
...
/**
 * 
 * @param {json} game - json object of game
 * @returns {json} - json object of game with only relevant fields
 * 
 */
